...
public class MainActivity extends BaseActivity implements WebViewCallback,
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    switch (item.getItemId()) {
      ...
      case R.id.menu_start_server:
        String pagePath,randomArticlePath,mainPagePath;
        pagePath = ZimContentProvider.getPageUrlFromTitle(ZimContentProvider.getZimFileTitle());
        randomArticlePath = ZimContentProvider.getRandomArticleUrl();
        mainPagePath = ZimContentProvider.getMainPage();
        Log.v("DANG","file Title : "+ZimContentProvider.getZimFileTitle());
        Log.v("DANG","page Path :"+pagePath);
        Log.v("DANG","CONTENT URI : "+ZimContentProvider.CONTENT_URI);
        Log.v("DANG","RANDOM ARTICLE : "+randomArticlePath);
        Log.v("DANG","Main Page : "+mainPagePath);
        Log.v("DANG","ZIM FILE PATH: "+ZimContentProvider.getZimFile());
        Uri uri = Uri.parse(ZimContentProvider.CONTENT_URI + pagePath);
        Log.v("DANG","DEBUG 1");
        ...
        Log.v("DANG","URI : "+uri.toString());
        Log.v("DANG","uri.getPath(): "+uri.getPath());
        Log.v("DANG","selectedFilePath: "+selectedFilePath);
  ...
  public boolean onPrepareOptionsMenu(Menu menu) {
    ...
    if (tabSwitcherRoot.getVisibility() == View.VISIBLE) {
      ...
      menu.findItem(R.id.menu_read_aloud).setVisible(false);
      menu.findItem(R.id.menu_start_server).setVisible(false);
    } else {
      ...
      if (getCurrentWebView().getUrl() == null ||
        ...
        menu.findItem(R.id.menu_random_article).setVisible(true);
        menu.findItem(R.id.menu_searchintext).setVisible(true);
        menu.findItem(R.id.menu_start_server).setVisible(true);
      }
    }
    return true;
  }
